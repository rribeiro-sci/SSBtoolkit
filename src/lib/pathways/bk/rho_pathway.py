################################################################################
##                      THE RHODOSPIN CASCADE VERSION 1.1                     ##
##                        last modification 22 May 2018                       ##
##      SOURCES:                                                              ##
##       1.	Invergo BM, Dell'Orco D, Montanucci L, et al (2014)               ##
##            A comprehensive model of the phototransduction cascade in       ##
##            mouse rod cells. Mol BioSyst 10:1481–1489                       ##
##       2.	Invergo BM, Montanucci L, Koch K-W, et al (2013)                  ##
##            Exploring the rate-limiting steps in visual phototransduction   ##
##            recovery by bottom-up kinetic modeling. Cell Communication      ##
##            and Signaling : CCS 11:36                                       ##
################################################################################
from pysb import *
...
#Set matplotlib environment up
%matplotlib inline

##Inputs
imp1 = input("Enter number of photoavtivated rhodopsins: ")
imp2 = input("Enter time: ")

## Start time
t0 = time.time()
t0_process = time.clock()
...
Monomer('GTP', ['b2'])
# Assuming only one alosteric bindig site (SINGLE ACTIVATION)
...
Initial(Rho(b1=None, p='O', s='i'), Parameter('Rho_i_0', 3.6E9 - Rho_activated))   # number |Total amount of R [Dell'orco et al. 2009]
Initial(Rho(b1=None, p='O',s='a'), Parameter('Rho_a_0', Rho_activated))
...
###### CALCIUM #####
'''The decreasinng in Ca2+ concentrations trigger the activity o two
...
# OBSERVABLES
Observable('obs1', GMP(b=None))
Observable('obs2', cGMP(b=None))

# RUNNING SIMULATION
t = pl.linspace(0, ttotal, num=1000) # (a,b,c); a is the starting time ; b is the total time simulated ; c is the number of points
simres = ScipyOdeSimulator(model, tspan=t).run()
yout = simres.all
pl.figure()
#pl.subplot(211)
pl.plot(t, yout['obs1'], label="obs1")
#pl.subplot(212)
pl.plot(t, yout['obs2'], label="obs2")
...
# conc= yout['obs1']
# conc2= yout['obs2']
# df = pd.DataFrame({"x" : t, "y" : conc})
# df2 = pd.DataFrame({"x": t, "y" :conc2})
# df.to_json(path_or_buf="/Users/rribeiro/Local/cascade/time_GMP_10.json")
# df.to_json(path_or_buf="/Users/rribeiro/Local/cascade/time_cGMP_10.json")
