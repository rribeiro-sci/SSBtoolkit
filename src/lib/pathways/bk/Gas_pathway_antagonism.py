...
def network(pKd_agonist,conc_agonist,pKd_antagonist, conc_antagonist, epsilon):


    Rtotal = 2000
    #from pKd to Kd
    Kd_anatagonist = (10**(-(float(pKd_antagonist)))) * 10**9
    Kd_agonist = ((10**(-(float(pKd_agonist)))) * 10**9)*(1+(conc_antagonist/Kd_anatagonist))

    #LR determination
    a = 1
    b = float(conc_agonist)+float(Rtotal)+Kd_agonist
    c = float(Rtotal)*float(conc_agonist)
    delta = (b**2) - (4*a*c)
...
list_of_observables=['obs_D1R','obs_D1RDA','obs_cAMP']
