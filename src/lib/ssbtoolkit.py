...
class get:
    ...
    def gprotein(uniprotID):
        ...
        #query HuTRdb
        #dbpath = os.path.join(os.getcwd(),'SSBColab-shared/src/databases/HuTRdb.sqlite3') #GOOGLE COLAB 
        dbpath = os.path.join(os.getcwd(),'src/databases/HuTRdb.sqlite3') #LOCAL
...
class binding:
    def __init__(self, receptor_conc, lig_conc_range, pKd):
        self.receptor_conc = receptor_conc
        self.lig_conc_range =  lig_conc_range
        self.pKd = pKd
...
class network:
    ...
    class activation:
        ...
        def set_simulation_parameters(self, **kwargs):
            ...
            self.affinity_values=kwargs.pop('affinity_values')
        ...
        def run(self):
            ...
            elif self.network_name==None: raise TypeError("network name undefined.")
            ...
            if self.network_name == 'Gz(Gi)': self.network_name = 'Gi'
            if self.network_name not in available_networks: raise Exception('Unvailable Network. Please, introduce it manually. Networs available: "Gs", "Gi", "Gq".')

            #Main function
            pathway = importlib.import_module('.'+self.network_name, package='src.lib.pathways')
            ...
            for ligand in self.ligands:
                ...
                for idx in range(len(self.lig_conc_range)):
                    ...
                    if self.kinetics == False:
                        ...
                        LR_conc_init = sharedfunctions.LR_eq_conc(self.receptor_conc, ligand_conc, 0, self.affinity_values[self.ligands.index(ligand)], 0)
                ...
                simulation_data[ligand_name] = {'sim_data':data,
                                            'label':ligand_name,}
                                            #'pkd':str(affinity_values[ligands.index(ligand)])}
        ...
        def analysis(self):
            ...
            if self.simulation_data == None: raise TypeError('There is no simulation data. network.stimulation.run() must be run first.')
            ...
            for ligand in self.simulation_data:
                ...
                if self.network_name == 'Gi' or self.network_name == 'Gz(Gi)':
                    metabolite='cAMP'
                    metabolite_conc_raw=[]
        ...
        def potency_to_dict(self):
                #dependencies
                if self.processed_data == None: raise TypeError('Simulation data unprocessed. network.stimulation.analysis() must be run first.')
        ...
        def show_potency(self):
            import pandas as pd
            data = network.inhibition.potency_to_dict(self)
    ...
    class inhibition:
        def __init__(self):
            ...
            self.antagonists_affinity=None
            self.network_name=None 
            self.receptor_conc=None 
            self.lig_conc_range=None 
            self.ttotal=None 
        ...
        def set_simulation_parameters(self, **kwargs):
            ...
            self.antagonists_affinities=kwargs.pop('antagonists_affinity')
            self.network_name=kwargs.pop('network_name')
            self.receptor_conc=kwargs.pop('receptor_conc') 
            self.lig_conc_range=kwargs.pop('lig_conc_range') 
            self.ttotal=kwargs.pop('ttotal') 
        ...
        def run(self):
            ...
            #Check inputs
            if self.agonist==None: raise TypeError("Agonist list undefined.")
            elif self.agonist_affinity==None: raise TypeError("agonist_affinity undifined.")
            elif self.antagonists==None: raise TypeError("Antaognists list undefined.")
            elif self.antagonists_affinities==None: raise TypeError("Affinity values undefined.")
            elif self.network_name==None: raise TypeError("network name undefined.")
            elif self.lig_conc_range.any() == False: raise TypeError("lig_conc_range undefined.")
        ...
        def analysis(self):
            #dependencies
            if self.simulation_data == None: raise TypeError('There is no simulation data. network.inhibition.run() must be run first.')
        ...
        def curve(self):
            #dependencies
            if self.processed_data == None: raise TypeError('Simulation data unprocessed. network.inhibition.analysis() must be run first.')
    ...
    class fitModel:
        ...
        def set_simulation_parameters(self, **kwargs):
            ...
            if 'network_name' in kwargs: 
                self._network_name = str(kwargs.pop('network_name'))
                print('network_name ->', self._network_name)
            else: raise TypeError("network_name undefined.")

            available_networks = ['Gs', 'Gi', 'Gq', 'OXTR_pathway']
            if self._network_name == 'Gz(Gi)': self._network_name = 'Gi'
            if self._network_name not in available_networks: raise Exception('Unvailable Network. Please, introduce it manually. Networs available: "Gs", "Gi", "Gq".')
        ...
        def run(self, **kwargs):
            ...
            #Main function
            pathway = importlib.import_module('.'+self._network_name, package='src.lib.pathways')
            self.simtime = pl.geomspace(0.00001, self._ttotal, num=self._nsteps) 

            #Simulation 1
            network_model = pathway.network(LR=None, kinetics=True, **self.network_parameters)
            sim1 = ScipyOdeSimulator(network_model, tspan=self.simtime,compiler='cython').run()
            ...
            def calc_ratio(self):
                ...
                #Simulation 2
                sim2 = ScipyOdeSimulator(pathway.network(**self.new_network_parameters), tspan=self.simtime, compiler='cython').run()
            ...
            for idx in range(self._maxiter):
                ...
                self.new_network_parameters={**self.network_parameters, **{self._target_parameter:pathway.defaultParameters[self._target_parameter]*self._seed}}
        ...
        def plotIterations(self):
            #dependencies
            if self._iteration == None: raise TypeError('Simulation data not exist. network.fitModel.run() must be run first.')
